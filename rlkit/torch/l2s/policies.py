...
class ReparamTanhMultivariateGaussianPolicy(Mlp, ExplorationPolicy):
    ...
    def sample(self, obs,return_prob=True):
        ...
        self.eval()
        action, mean, log_std, log_prob, expected_log_prob, std, mean_action_log_prob, pre_tanh_value = self.forward(obs, return_log_prob=return_prob)
        #print(action.size())
...
class L2SGaussianPolicy(Mlp, ExplorationPolicy):
    def __init__(
            self,
            hidden_sizes,
            obs_dim,
    ...
    def get_std(self, obs_np, deterministic=False):
        res = self.eval_np(obs_np[None], deterministic=deterministic)[5]
        return res[0,:]
...
class L2SDiscretePolicy(Mlp, ExplorationPolicy):
    """
    Usage:
    ...
    def get_actions(self, obs_np, deterministic=False):
        res = self.eval_np(obs_np)[0] #[batch,1]
        res = res[:,0] #[batch,1]
        res = np.eye(self.action_dim)[res] #[batch,act_dim]
        return res
    ...
    def forward(
        ...
        if deterministic:
            log_prob, idx = torch.max(log_probs, 1) #idx: [batch]
            idx = torch.unsqueeze(idx,dim=-1) #idx: [batch]
            return (idx, 1)
        else:
            # Using Gumbel-Max trick to sample from the multinomials
            u = torch.rand(pre_act.size(), requires_grad=False)
            gumbel = -torch.log(-torch.log(u)) # [batch,obs_dim]
            _, idx = torch.max(gumbel + pre_act, 1) # idx[]
            ...
            # print(idx)
            # print(log_prob)
            return (idx, log_prob, 1,1,1,1,1)
    
    def get_log_pis(self, obs):
        return super().forward(obs)


class TestBCPolicy(L2SGaussianPolicy):
    def __init__(
            self,
            **kwargs
    ):
    ...
    def get_actions(self, obs_np, deterministic=False):
        return self.eval_np(obs_np, deterministic=deterministic)[0]/1000.0


    def get_log_prob(self, obs, acts, return_normal_params=False):
        acts = acts*1000.0
        return super().get_log_prob(obs, acts, return_normal_params)
