...
class SoftActorCritic(Trainer):
    ...
    def train_step(self, batch):
        ...
        # print(q1_pred.size())
        # q3 = torch.min(torch.cat([q1_pred,q2_pred],dim=-1),dim=-1).values
        # print(q3)
        # print(q3.size())
        # exit(1)
        """
        ...
        with torch.no_grad():
            if self.q_loss_type==0:
                ...
                max_q_new_actions = torch.max(q1_new_acts, q2_new_acts)
                v_target = self.minmax_weight*q_new_actions + (1.0-self.minmax_weight)*max_q_new_actions  - log_pi
    ...
    def get_min_Q(self,obs,actions):
        q1_acts = self.qf1(obs, actions)
        q2_acts = self.qf2(obs, actions)
        q_new_actions = torch.min(q1_acts, q2_acts)
